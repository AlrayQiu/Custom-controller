...
float MPU_Get_Temperature(void)
{
    ...
    MPU_Read_Len(0, MPU_TEMP_OUTH_REG, 2, buf);
    raw  = (buf[0] << 8) | buf[1];
...
{
    uint8_t buf[6], res;
    res = MPU_Read_Len(0, MPU_GYRO_XOUTH_REG, 6, buf);
    if (res == 0) {
...
{
    uint8_t buf[6], res;
    res = MPU_Read_Len(0, MPU_ACCEL_XOUTH_REG, 6, buf);
    if (res == 0) {
...
{
    extern I2C_HandleTypeDef hi2c1;
    d = HAL_I2C_Mem_Read(&hi2c1, MPU_READ, reg, I2C_MEMADD_SIZE_8BIT, buf, len, 0xfff);
    HAL_Delay(1);

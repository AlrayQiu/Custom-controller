...
// 设置管理
#define setUnlock    can_write_reg(addr, 0x69u, (0x88u << 8) | 0xB5u)
#define saveSettings can_write_reg(addr, Jm_RegAddr_Save, (0x00u << 8 | 0xB5u))
//
// 宏定义结束
...
BOOL Jyme02_Can_Init(CAN_HandleTypeDef *phcan, uint32_t filterfifo)
{
    ...
    filter.FilterScale = CAN_FILTERSCALE_32BIT; // 滤波器位宽

    filter.FilterIdHigh         = 0x0000; // 基本ID放入到STID中
    filter.FilterIdLow          = 0x0000;
    filter.FilterMaskIdHigh     = 0x0000;           // 这里设置接收任意ID的CAN数据
    filter.FilterMaskIdLow      = 0x0000;           // 0表示接收来自发送端任意ID的数据

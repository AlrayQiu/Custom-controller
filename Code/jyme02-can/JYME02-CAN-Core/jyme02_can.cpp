...
// 设置管理
#define setUnlock    can_write_reg(addr, 0x69u, (0xB5u << 8) | 0x88u)
#define saveSettings can_write_reg(addr, Jm_RegAddr_Save, (0x00u << 8 | 0x00u))
#define reset        can_write_reg(addr, Jm_RegAddr_Save, (0x00u << 8 | 0x01u))
#define restart      can_write_reg(addr, Jm_RegAddr_Save, (0x00u << 8 | 0xffu))
//
// 宏定义结束
...
void Jyme02_Can_Reset(Device_Addr addr)
{
    setUnlock;
    reset;
    restart;
